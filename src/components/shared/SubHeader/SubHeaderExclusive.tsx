...
export default function SubHeaderExclusive() {
  const [settings, setSettings] = useState<ExclusiveSettings | null>(null);
  const [siteData, setSiteData] = useState<SiteData>({});
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    // Fetch exclusive offer settings
  ...
  const handleScroll = () => {
    const el = document.getElementById("exclusive-registration-form");
    if (el) {
      el.scrollIntoView({ behavior: "smooth", block: "start" });
    }
  };

  // Don't show if inactive, expired, or still loading
        ...
        <div className="flex flex-col md:flex-row items-center justify-between pt-2 md:pt-3 md:pb-1">
          <div className="flex items-center justify-center md:items-start flex-col text-center md:text-left md:px-2">
            ...
            <p className="text-[12px] md:text-[15px] text-purple-100 font-medium opacity-80 uppercase tracking-tighter pb-[2px] md:pb-0">
              {settings.description || "Exclusive Limited Time Offer"}
            </p>
          </div>
